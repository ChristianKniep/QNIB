...
class checks(object):
    ...
    def ibnetdiscover(self):
        ...
        for line in lines:
            ...
            reg = "\[(\d+)\]\(([a-f0-9]+)\).*\"S-[0]*[a-f0-9]+\"\[(\d+)\].*lid (\d+) lmc.*lid (\d+) (\d+)x([A-Z]DR).*"
            mat = re.match(reg, line)
            if mat and self.sysimgguid:
                    # Port gefunden
            ...
            if self.matchChassis(line):
                continue
            # sysimgguid (mit/ohne Chassis)
            if self.matchSystem(line):
                continue
            # Modulare Switches
            if self.matchSwitch(line):
                continue
            # CA
            if self.matchCA(line):
                continue
        for line in lines:
            if self.matchContinue(line):
                continue
            # Modulare Switches
            if self.matchSwitch(line, True):
                continue
            # switchport
            if self.matchSwitchPortGen(line):
                continue
    ...
    def matchChassis(self, line):
        ...
        if m:
            ...
            cId = self.db.getIns_ID('chassis', "c_guid='%s'" % str(chassisguid),
                ['c_guid', 'c_nr', 'c_name'],
                [str(chassisguid), self.cNr,
                 self.chassisNames[chassisguid]['name']])
        return m
    ...
    def matchSystem(self, line):
        ...
        m = re.match(reg, line)
        if m:
                self.sys = libTopology.parseSystem(self.db, self.opt, self.cfg,
                                                   self.sysGuids)
    ...
    def evalMatches(self):
        ...
        # Ich baue einen Pfad auf, solange es Kanten in Reichweite gibt,
        # die nicht zu einem Kreis gehören.
        # Wenn ich zu einem Knoten komme, den ich schon im Pfad habe,
        # dann markiere ich alle Kanten des Pfad als Kreiskanten (link.circle)
    ...
    def evalHistory(self):
        ...
        while True:
            ...
            if n_id == prev_n_id:
                ...
                n_state_new = self.eval_states(prev_n_id, prev_n_name,
                                               prev_n_status, p_stats)
            ...
            if len(res) == 0:
                ...
                break
            self.log.debug("%-10s: N:%-5s->%-5s || P:%-5s" % \
                           (n_name, prev_n_status, n_status, p_status), 2)
    ...
    def recursiv(self, node):
        ...
        self.deb(node.getLids(), "Neue Rekursion von '%s' mit Kindern '%s'" % \
                 (node.name, ",".join(kinder)), 'l', 1)
        self.actP.addNode(node)
        for child, links in node.nLinks.items():
            s_lids = node.getLids()
            s_lids |= child.getLids()
            self.deb(s_lids, "Betrachte Kind '%s' mit Verbindungen: " % \
                     (child), 'l', 1)
            for link in links:
                self.deb(link.getLids(), link, 'l', 1)
            if not child.isSwitch():
                node.comp_cnt += 1
                self.deb(child.getLids(), "%s ist kein Switch, weiter" % \
                         child.name, 'l', 2)
                continue
            ...
            if self.actP.linkIn(links):
                self.deb(link.getLids(), "%s - Link schon drin %s" % \
                  (link.getLids(), [link.__str__() for link in links]), 'l', 1)
                continue
            if child.seen:
                self.deb(child.getLids(),
                         "%s Kind '%s' über Link '%s' schon gesehen >> %s" % \
                ...
                if not True:
                    self.deb(child.getLids(),
                             "Neuer Kreis schliesst sich", 'l', l)
            else:
